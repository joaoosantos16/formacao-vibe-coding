      ...
      <table className="min-w-full text-xs">
        <thead className="text-left text-slate-500">
          <tr>
            ...
            <th className="px-3 py-2 font-medium text-right">Variables</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-100">
          {rows.map((row) => (
            <tr key={row.team}>
              ...
              <td className="px-3 py-1.5 text-right tabular-nums text-slate-600">{row.days.toFixed(1)}</td>
              <td className="px-3 py-1.5 text-right tabular-nums text-slate-600">{formatK(row.valueK)}</td>
              <td className="px-3 py-1.5 text-right tabular-nums text-slate-600">{formatK(row.variablesK)}</td>
            </tr>
          ))}
        </tbody>
        <tfoot className="border-t border-slate-200 font-semibold text-slate-800">
          <tr>
            ...
            <td className="px-3 py-2 text-right tabular-nums">{formatK(totals.valueK)}</td>
            <td className="px-3 py-2 text-right tabular-nums">{formatK(totals.variablesK)}</td>
          </tr>
        </tfoot>
      </table>
    </div>
