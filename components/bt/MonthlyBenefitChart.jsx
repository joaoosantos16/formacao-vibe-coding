...
export default function MonthlyBenefitChart({ months, byMonth, byMonthPlan }) {
  ...
  const barW = Math.min(22, ((W - L - R) / N) * 0.6);
  ...
  const gridLines = Array.from({ length: 5 }, (_, i) => {
    ...
    return (
      <g key={i}>
        ...
        <text x={L - 6} y={yy + 3.5} fontSize="9.5" fill="#6B6B7A" textAnchor="end">{axisEur(v, mx)}</text>
      </g>
    );
  });
  ...
  const bars = months.map((m, i) => {
    ...
    return <rect key={m.key} x={x(i) - barW / 2} y={yTop} width={barW} height={h} rx={2} fill="#0E7A68" />;
  });
  ...
  const labels = months.map((m, i) => (
    (N <= 14 || i % 2 === 0) ? (
      <text key={m.key} x={x(i)} y={H - 16} fontSize="9" fill={byMonth[m.key] ? '#333' : '#BDBDBD'} textAnchor="middle">
        {m.label}
      </text>
    ) : null
  ));
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ minWidth: W }}>
      ...
      <line x1={L} y1={y(0)} x2={W - R} y2={y(0)} stroke="#6B6B7A" />
      {labels}
    </svg>
  );
}
