...
// Setor / Indústria (subsetor) — os MESMOS valores reais que existem em
// projetos.setor/.subsetor (ver 75 projetos Spark Week) e que o
// Benchmarking usa em STANDARD_MACRO_SECTORS/STANDARD_INDUSTRIES
// (app/benchmarking/data.js). Antes esta lista era um catálogo à parte,
// pequeno e sem nenhuma correspondência com os dados reais — corrigido
// para as duas páginas baterem certo (pedido do utilizador, 26/08).
// Indústria não está realmente dependente do setor nos dados reais, por
// isso SECTOR_SUBSECTORS mapeia todos os setores para a mesma lista
// completa — mantém a UI de "duas dropdowns" sem inventar uma hierarquia
...
export const SECTOR_SUBSECTORS = Object.fromEntries(SECTORS.map((s) => [s, INDUSTRIES]));
...
export async function createProject(data) {
  ...
  const { error } = await supabase.from('projetos').insert({
    ...
    area_negocio: data.businessArea ?? null,
...
export async function updateProject(id, patch) {
  ...
  if ('businessArea' in patch) payload.area_negocio = patch.businessArea;
  if ('country' in patch) payload.pais = patch.country;
...
export async function addKpiToProject(id, kpi) {
  const { data, error } = await supabase
    ...
    .single();
  if (error) throw error;
  await logAudit(id, data.id, 'KPI added', null, kpi.name);
  return data.id;
}
...
export async function setMeasurement(id, kpiId, period, value) {
  ...
  await logAudit(id, kpiId, `Act ${period}`, null, value === null ? '(deleted)' : String(value));
}
...
export async function setPlanOverride(projetoCodigo, kpiId, mes, value) {
  ...
  await logAudit(projetoCodigo, kpiId, `Plan ${mes}`, null, value === null ? '(automatic ramp)' : String(value));
}
...
export async function setVolumeOverride(projetoCodigo, kpiId, mes, value) {
  ...
  await logAudit(projetoCodigo, kpiId, `Volume ${mes}`, null, value === null ? '(annual volume / 12)' : String(value));
}
...
export async function setAtualOverride(projetoCodigo, kpiId, mes, value) {
  ...
  await logAudit(projetoCodigo, kpiId, `Actual ${mes}`, null, value === null ? '(captures)' : String(value));
}
