...
export function isAtualOverridden(kpi, mk) {
  return Boolean(kpi.atualOverrides && kpi.atualOverrides[mk] !== undefined && kpi.atualOverrides[mk] !== '');
}
...
export function calcKpi(project, kpi, months) {
  ...
  months.forEach((m) => {
    const override = kpi.atualOverrides ? kpi.atualOverrides[m.key] : undefined;
    if (override !== undefined && override !== null && override !== '') {
      const ov = Number(override);
      if (!Number.isNaN(ov)) act[m.key] = ov;
    }
  });
