...
// initiative name. em/occPct/varK/varStatus/greenDays line up 1:1 with
// that section's rows for the same underlying project.
...
export function getProductivityTopFree() {
  return [
    { project: 'ACME-201-POR', em: 'J. Rosa', occPct: 62, varK: 45.0, varStatus: 'OK', greenDays: 38, critical: false, continuidade: null },
    { project: 'NWFD-202-POR', em: 'M. Alves', occPct: 88, varK: 32.5, varStatus: 'OK', greenDays: 55, critical: false, continuidade: 'Yes' },
    { project: 'CTSO-203-POR', em: 'T. Nunes', occPct: 34, varK: 58.0, varStatus: 'NOK', greenDays: 12, critical: true, continuidade: 'No' },
    { project: 'FBRK-204-POR', em: 'J. Rosa', occPct: 71, varK: 21.0, varStatus: 'OK', greenDays: 44, critical: false, continuidade: null },
    { project: 'GLBX-205-POR', em: 'S. Costa', occPct: 45, varK: 40.0, varStatus: 'NOK', greenDays: 20, critical: true, continuidade: null },
    { project: 'INIT-206-POR', em: 'M. Alves', occPct: 91, varK: 27.0, varStatus: 'OK', greenDays: 58, critical: false, continuidade: 'Yes' },
    { project: 'UMBR-207-POR', em: 'T. Nunes', occPct: 39, varK: 63.0, varStatus: 'NOK', greenDays: 15, critical: true, continuidade: 'No' },
  ];
}
...
//
// Billing days balance (right axis) — a burn-down, the standard way to
// control pace against a budget: "Theoretical" is a straight line from the
// full day budget at project start to zero at project end, spread over the
// project's real duration (from the Variables section's start/end dates,
// not just this 12-week window). "Real consumption" is the actual balance
// up to "now" — it drifts away from Theoretical when the project runs
// hotter or colder than planned, and that gap is the point: it's what lets
// the team see if they're burning billing days faster than the plan.
const WEEKS = 12;
const NOW_WEEK_INDEX = 7; // week 8 of 12
const TOTAL_BUDGET_DAYS = 60;
...
export function getProjectWeeklySeries(projectCode) {
  ...
  const variablesRow = getVariablesRows().find((v) => v.projectCode === row.project);
  ...
  const projectWeeks = variablesRow
    ? Math.max(1, Math.round((new Date(variablesRow.end) - new Date(variablesRow.start)) / (7 * 24 * 60 * 60 * 1000)))
    : WEEKS;
  const theoreticalBillingDays = Array.from({ length: WEEKS }, (_, w) =>
    Math.max(0, Math.round(TOTAL_BUDGET_DAYS * (1 - w / projectWeeks)))
  );

  // >0 when occupation runs hotter than a 75% baseline (burns days faster
  // than plan), <0 when it runs colder (burns slower).
  const paceFactor = (row.occPct - 75) / 100;
  const realBillingDays = [];
  for (let w = 0; w <= NOW_WEEK_INDEX; w++) {
    const drift = TOTAL_BUDGET_DAYS * paceFactor * 0.3 * (w / NOW_WEEK_INDEX);
    realBillingDays.push(Math.max(0, Math.round(theoreticalBillingDays[w] - drift)));
  }
  ...
  return Array.from({ length: WEEKS }, (_, w) => ({
    ...
    theoreticalBillingDays: theoreticalBillingDays[w],
    realBillingDays: w <= NOW_WEEK_INDEX ? realBillingDays[w] : null,
  }));
}
