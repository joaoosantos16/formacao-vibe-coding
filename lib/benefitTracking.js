...
// These are individual consultants, not teams — see getConsultantLevels()
// for the real Kaizen career ladder these people sit on.
export function getHoshinTeamRows() {
  return [
    { consultant: 'pt ppereira', days: 360.5, valueK: 883.22, variablesK: 79.5 },
    { consultant: 'pt msoares', days: 401.0, valueK: 866.42, variablesK: 78.0 },
    { consultant: 'pt asaraiva', days: 369.5, valueK: 804.48, variablesK: 72.4 },
    { consultant: 'pt acunha', days: 329.5, valueK: 784.44, variablesK: 70.6 },
    { consultant: 'pt joaosilva', days: 327.0, valueK: 650.89, variablesK: 58.6 },
    { consultant: 'pt pcarvalho', days: 310.0, valueK: 644.34, variablesK: 58.0 },
    { consultant: 'pt jxavier', days: 286.0, valueK: 618.37, variablesK: 55.7 },
    { consultant: 'pt no team', days: 222.5, valueK: 525.05, variablesK: 47.3 },
    { consultant: 'pt martalemos', days: 219.5, valueK: 464.05, variablesK: 41.8 },
    { consultant: 'pt aandion', days: 211.5, valueK: 411.93, variablesK: 37.1 },
  ];
}
...
// occupation first. em/project line up with the Variables section's rows.
// varK/varStatus mirror that section's Potential/Status for context.
...
export function getProductivityTopFree() {
  return [
    { project: 'Packaging OEE', em: 'J. Rosa', occPct: 62, varK: 45.0, varStatus: 'OK', greenDays: 38, critical: false, continuidade: null },
    { project: 'Warehouse Flow', em: 'M. Alves', occPct: 88, varK: 32.5, varStatus: 'OK', greenDays: 55, critical: false, continuidade: 'Yes' },
    { project: 'Line Changeover', em: 'T. Nunes', occPct: 34, varK: 58.0, varStatus: 'NOK', greenDays: 12, critical: true, continuidade: 'No' },
    { project: 'Inventory Accuracy', em: 'J. Rosa', occPct: 71, varK: 21.0, varStatus: 'OK', greenDays: 44, critical: false, continuidade: null },
    { project: 'Delivery Reliability', em: 'S. Costa', occPct: 45, varK: 40.0, varStatus: 'NOK', greenDays: 20, critical: true, continuidade: null },
    { project: 'Store Replenishment', em: 'M. Alves', occPct: 91, varK: 27.0, varStatus: 'OK', greenDays: 58, critical: false, continuidade: 'Yes' },
    { project: 'Batch Setup Time', em: 'T. Nunes', occPct: 39, varK: 63.0, varStatus: 'NOK', greenDays: 15, critical: true, continuidade: 'No' },
  ];
}

// Weekly occupation trend for one project, deterministic mock (no real KIM/
// Planner feed yet). "Real" covers weeks up to "now"; "Objective" is the
// flat target band; "Forecast" continues from "now" in a straight line,
// extrapolating the slope of the last few Real weeks. billingDaysBalance is
// a second, differently-scaled series (remaining billable days) — plotted
// on its own axis in the chart, not mixed into the % scale.
const WEEKS = 12;
const NOW_WEEK_INDEX = 7; // week 8 of 12

export function getProjectWeeklySeries(projectName) {
  const rows = getProductivityTopFree();
  const row = rows.find((r) => r.project === projectName) ?? rows[0];
  const objective = 85;
  const startOcc = 90;
  ...
  const totalBillingDays = 60;
  const billingDaysBalance = [];
  for (let w = 0; w < WEEKS; w++) {
    const consumedRatio = w <= NOW_WEEK_INDEX ? w / NOW_WEEK_INDEX : 1;
    const base = totalBillingDays * (1 - consumedRatio * (row.occPct / 100));
    billingDaysBalance.push(Math.max(0, Math.round(base)));
  }
  ...
  return Array.from({ length: WEEKS }, (_, w) => ({
    ...
    billingDaysBalance: billingDaysBalance[w],
  }));
}
...
export function getConsultants() {
  return [
    { consultant: 'pt ppereira', level: 'Senior Consultant', occPct: 82, fridayUtilPct: 65 },
    { consultant: 'pt msoares', level: 'Manager', occPct: 88, fridayUtilPct: 70 },
    { consultant: 'pt asaraiva', level: 'Project Leader', occPct: 75, fridayUtilPct: 58 },
    { consultant: 'pt acunha', level: 'Consultant', occPct: 69, fridayUtilPct: 52 },
    { consultant: 'pt joaosilva', level: 'Senior Project Leader', occPct: 91, fridayUtilPct: 74 },
    { consultant: 'pt pcarvalho', level: 'Consultant', occPct: 63, fridayUtilPct: 48 },
    { consultant: 'pt jxavier', level: 'Consultant', occPct: 55, fridayUtilPct: 40 },
    { consultant: 'pt martalemos', level: 'Consultant', occPct: 48, fridayUtilPct: 35 },
    { consultant: 'pt aandion', level: 'Senior Consultant', occPct: 71, fridayUtilPct: 55 },
  ];
}
